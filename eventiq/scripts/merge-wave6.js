...
/**
 * merge-wave6.js — Merge 50 researched companies from Wave 6 batch files
 * into src/data/all-companies.json
 *
 * Usage: node scripts/merge-wave6.js
 */
...
const BATCH_FILES = [
  path.join(__dirname, 'p4-wave6-batch-1.json'),
  path.join(__dirname, 'p4-wave6-batch-2.json'),
  path.join(__dirname, 'p4-wave6-batch-3.json'),
  path.join(__dirname, 'p4-wave6-batch-4.json'),
  path.join(__dirname, 'p4-wave6-batch-5.json'),
];
...
function main() {
  ...
  for (const batchCompany of allBatchCompanies) {
    ...
    // Category normalization — force lowercase
    if (batchCompany.category) {
      const normalized = batchCompany.category.toLowerCase();
      if (normalized !== batchCompany.category) {
        categoryCorrections++;
  ...
  console.log(`Category corrections (case): ${categoryCorrections}`);
